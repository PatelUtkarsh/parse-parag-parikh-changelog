...
class ParseCommand extends Command
{
    ...
    public function configure(): void
    {
        $this->setName('parse')
             ...
             ->setHelp(
                 'This command help you generate diff of share changes per month for a PPFAS mutual fund'
             )
             ->addArgument(
                 'month-diff-x',
                 InputArgument::OPTIONAL,
                 'For comparing old sheet as X, As if Y - X.',
                 2
             )
             ->addOption(
                 'fund-name',
                 ...
                 'Pick from ' . join(',', array_keys(self::SHEET_MAP)),
                 'tax'
             )
             ->addOption(
                 'month-diff-y',
    ...
    public function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $new_path = $this->downloadHandler($month_new);
        ...
        $old_sheet = $this->getExcelColumnMap($old_path, $option);
        $new_sheet = $this->getExcelColumnMap($new_path, $option);
    ...
    public function downloadHandler(int $month): string|false
    {
        $url = $this->generateUrl($month, 'xlsx');
        $this->output->isVerbose() && $this->output->writeln('Downloading file: ' . $url);
        $new_path = $this->downloadSpreadsheetInTempDir($url);
        if (! $new_path) {
            $this->output->isVerbose() &&
            $this->output->writeln('Unable to download xlsx file, retrying with xls extension: ' . $url);
            // retry download with xls extension.
            $new_url = $this->generateUrl($month);

            return $this->downloadSpreadsheetInTempDir($new_url);
        }
        ...
        try {
            ...
            for ($row = 1; $row <= 20; $row++) {
                ...
                foreach ($rowData as $col => $value) {
                    if (is_string($value)) {
                        ...
                        if (
                            strpos($lowerValue, 'company') !== false ||
            ...
            foreach ($worksheet->getRowIterator() as $row_object) {
                ...
                if (strcasecmp($column_b_value, self::SECTION_TERMINATOR) === 0) {
                    ...
                    $this->output->isVerbose() &&
                    $this->output->writeln("Found GRAND TOTAL at row {$row_number}, stopping data extraction");
                    break;
                }
                ...
                foreach (self::TARGET_SECTION_HEADERS as $display_header => $normalized_key) {
                    if (strcasecmp($column_b_value, $display_header) === 0) {
                        ...
                        $section_found                  = true;
                        $this->output->isVerbose() &&
                        $this->output->writeln("Found section header: {$display_header} -> {$normalized_key}");
                        break;
                    }
                }
                ...
                if ($current_section_normalized_key !== null) {
                    ...
                    $this->output->isVerbose() &&
                    $this->output->writeln(
                        "Added to {$current_section_normalized_key}: {$name_value} ({$percent_numeric})"
                    );
                }
            }
            ...
            if (empty($sections_data) && $try < 1 && is_array(self::SHEET_MAP[ $sheetCode ])) {
                $this->output->isVerbose() &&
                $this->output->writeln(
                    'Unable to find any sectioned data in sheet: ' . $sheetName . ' retrying with next sheet name.'
                );

                return $this->getExcelColumnMap($filename, $sheetCode, $try + 1);
            }
    ...
    public function downloadSpreadsheetInTempDir(string $url): string|false
    {
        $userAgents = [
            <<<'EOF'
...
,
    ...
    public function generateUrl(int $month_diff, string $extension = 'xls'): string
    {
        ...
        $last_day_of_month = $this->getLastDateOfDatetime($current_date);
        $year              = $last_day_of_month->format('Y');
        $month             = $last_day_of_month->format('F');
        $day               = $last_day_of_month->format('d');

        return sprintf(
            "https://amc.ppfas.com/downloads/portfolio-disclosure/%s/PPFAS_Monthly_Portfolio_Report_%s_%s_%s.%s",
            $year,
            $month,
            $day,
            $year,
